...
## D046 — 2026-09-25 — Tool agent simetris (masuk & keluar) + USDC simulasi untuk agent
- Masalah: agent wallet bisa masuk (`agent_create_index`, `agent_join`, `agent_propose_update`) tetapi tidak bisa keluar/merapikan: tidak ada redeem, klaim fee, apply/cancel update tertunda, dan USDC harus diklaim manual di `/faucet` ke alamat agent.
- Keputusan (kontrak: daftar tool MCP §7.6 bertambah, aditif; program tidak berubah):
  - `agent_redeem {index, shares? | pct? (1–100, default 100), toUsdc=true}`: memakai `zapOut` SDK; cek saldo share ATA agent (tidak punya share / melebihi saldo → error jelas). Tidak ada cek `MCP_MAX_USDC_PER_ACTION` karena tidak ada USDC yang dibelanjakan. Hasil: share yang di-redeem, sisa share, USDC diterima (selisih saldo).
  - `agent_claim_fees {index}`: `claimFeesIxs` dengan `FeeKind.Creator` bila agent kreator index, dan `FeeKind.Parent` (+ parent index) bila agent kreator parent-nya; keduanya dalam satu transaksi (ATA share dibuat sekali). Program meng-accrue dulu saat klaim. Jumlah = selisih saldo share agent, nilai USD dari `sharePriceLive`. Tanpa owed dan tanpa management fee → tidak mengirim transaksi.
  - `agent_apply_update {index}`: program membolehkan siapa pun sebagai payer, tetapi tool dibatasi ke kreator/manager agar agent otonom tidak menyentuh index orang lain. Sebelum `eta` → error berisi waktu ETA, tanpa transaksi.
  - `agent_cancel_update {index}`: kreator saja (`CreatorOnly` di program).
  - `agent_get_test_usdc {amount=1000}`: `faucetIxs` (mock market) ditandatangani agent; maks = `CLUSTER_PARAMS[cluster].faucetMaxUsdc` per permintaan; cek SOL (fee, + sewa ATA USDC bila belum ada).
  - Web `POST /api/me/agents/[wallet]/fund` menerima body `{asset?: "SOL"|"USDC", amount?}` (default/body kosong = SOL, respons lama `{ok, sol}` tetap). USDC: pemilik saja, seed agent didekripsi (`decryptSecret`) dan alamat turunan dicocokkan, agent menandatangani faucet sendiri; SOL kurang → 400 "Fund SOL first: the agent pays the transaction fee". Default 1.000, maks per permintaan = min(faucet max cluster, 10.000), batas 10.000 USDC/agent/hari lewat `faucet_claims` kind `USDC`; pembayaran diserialkan per proses. Respons `{ok, usdc, signature}`.
- Untuk runner otonom (`agent:loop`): `agent_apply_update`, `agent_cancel_update`, `agent_claim_fees` boleh; `agent_redeem` dan `agent_get_test_usdc` tidak (menarik dana / mencetak dana baru harus diminta manusia).
- Alternatif ditolak: redeem via intent `/sign` (agent sudah memegang share-nya sendiri), faucet USDC dibayar admin (agent tetap butuh SOL untuk aksi berikutnya, jadi syarat SOL lebih jujur).

## D047 — 2026-09-25 — Autopilot terhosting: worker menjalankan agent milik user
- Masalah: agent otonom (`agent:loop`, D044) hanya jalan selama laptop user menyala. User ingin agent miliknya (D045) berjalan sendiri dari server.
- Keputusan:
  - Siklus keputusan dipindah ke modul `apps/mcp/src/autopilot.ts` (`@repo/mcp/autopilot`), dipakai bersama `scripts/agent-loop.ts` (MCP HTTP) dan worker (server MCP in-process lewat `InMemoryTransport.createLinkedPair()` + `createServer({ agent })`, tanpa HTTP). LLM lewat chat completions kompatibel OpenAI (`AGENT_LLM_*`, fallback `OPENROUTER_*`), fungsi chat dapat diinjeksi (test memakai LLM palsu).
  - Kebijakan tool = **allowlist**: baca (`list_assets`, `list_indexes`, `get_index`, `get_index_performance`, `get_leaderboard`, `get_portfolio`, `get_feed`), `simulate_rebalance`, `agent_info`, tulis `agent_rebalance`, `agent_propose_update`, `agent_apply_update`, `agent_cancel_update`, `agent_claim_fees`, `agent_post`. Tidak pernah: `build_*`, `agent_create_index`, `agent_join`, `agent_redeem`, `agent_get_test_usdc`, `agent_register`. Tool yang belum ada di server diabaikan. `agent:loop` ikut memakai allowlist ini (sebelumnya denylist; `agent_redeem` kini tidak lagi ditawarkan).
  - Scope: tool tulis dengan argumen `index` hanya boleh ke index yang dibuat/dikelola agent (disaring `indexes` bila diisi). Batas tetap: ≤12 panggilan tool/siklus, nominal ≤ `MCP_MAX_USDC_PER_ACTION`, status post tanpa aksi ≤1/index/hari. Strategi owner (≤1000 karakter) masuk system prompt di dalam `<owner_strategy>` sebagai instruksi owner yang tidak bisa menambah tool/scope/batas.
  - DB: `agent_autopilot` (pengaturan + jadwal; tambahan dari spesifikasi: `run_requested` untuk "run now" yang juga jalan saat autopilot mati, `last_manual_run_at` untuk batas 1 run manual/5 menit), `agent_runs` (log, 50 terbaru per agent), `worker_status` (heartbeat; web menghitung `available` dari heartbeat ≤5 menit karena kunci LLM tidak ada di env web/Vercel).
  - Penguncian: klaim `FOR UPDATE SKIP LOCKED` + `next_run_at` dimajukan + baris `running` dibuat dalam satu transaksi; run `running` >10 menit dianggap macet (worker berhenti) dan ditutup `error`. Timeout keras per run 120 s (abort + batas tunggu). 429 berulang dari LLM → loop jeda 5 menit.
  - Keamanan: kunci LLM dan `AGENT_KEY_SECRET` tidak pernah dicatat (redaksi di log dan di ringkasan run); kunci LLM hanya di mesin worker (tidak disalin `vercel:env`). Program vault tetap penjaga akhir.
- Alternatif ditolak: worker memanggil `/api/mcp` lewat HTTP dengan API key (butuh key mentah yang tidak disimpan server), cron Vercel (batas durasi fungsi, kunci LLM harus di Vercel), denylist tool (tool baru otomatis terbuka).

## D048 — 2026-09-25 — Mode asisten: kelola index lewat MCP (intent `/sign`)
- Masalah: di mode asisten (AI menyiapkan, user menandatangani di `/sign`) user hanya bisa create/clone/join/redeem. Mengelola index miliknya sendiri (ubah bobot/fee/strategi, apply/cancel update, pause, manager, klaim fee) harus lewat halaman Manage.
- Keputusan (kontrak: tool MCP §7.6 dan jenis intent bertambah, aditif; program & skema DB tidak berubah):
  - Tool baru `build_propose_update`, `build_apply_update`, `build_cancel_update`, `build_set_paused`, `build_set_managers`, `build_claim_fees`. Semua intent satu langkah (model progres D036 tetap: `next = null`).
  - Validasi di MCP (`apps/mcp/src/manage.ts`, murni & dites) sama dengan UI Manage: fee ≤ 5/1/1 %, slippage 0,5–5 %, drift > 0 dan ≤ 50 %, periode ≤ 365 hari, cooldown ≤ 30 hari, ≤ 10 aset, hanya aset listed & bukan benchmark (`investableMint`), tanpa duplikat, index follower tidak boleh ubah bobot. Aset yang masih punya saldo tapi tidak disebut otomatis dipertahankan di 0 % (sama dengan `agent_propose_update`). Bagian yang tidak berubah dibuang; tidak ada perubahan sama sekali → error. Fee-only yang turun ditandai `immediate` (langsung berlaku di program).
  - Intent diikat ke wallet yang diterima program: kreator (propose/cancel/pause/managers/klaim fee kreator) atau kreator parent (royalti clone; `index` = clone). `build_apply_update` tanpa ikatan (siapa pun boleh jadi payer) tetapi ditolak bila tidak ada pending atau ETA belum lewat (pesan berisi ETA). Parameter `wallet` opsional: bila bukan wallet yang berhak → error "Not allowed (403)" tanpa membuat link.
  - `build_set_managers {managers[]}` mengganti seluruh daftar (≤ 3, `[]` = hapus semua); entri boleh alamat atau nama agent terdaftar (`/api/agents`, `agentName`/`handle`); balasan menampilkan sebelum → sesudah.
  - Server (`lib/server/steps.ts` `manageStep`, dipanggil `POST /api/intents/[id]/tx`) memeriksa ulang on-chain sebelum membangun transaksi: signer = kreator / kreator parent (403 dengan pesan jelas), tidak ada rebalance ticket (409), pending/ETA untuk apply/cancel, status pause sudah sama (409), tidak ada yang bisa diklaim (409). Aset 0 % dihitung ulang dari saldo terbaru. `state.index` diisi sehingga `/sign` menautkan ke index setelah selesai.
  - `/sign`: `describe()` menampilkan baris manusiawi per jenis (bobot/fee/strategi via `pendingLines`, "Kept at 0% until sold", "Applies" sesuai timelock, before/after manager, owed shares); overlay transaksi satu fase "Confirm the transaction". Testid lama tetap.
- Alternatif ditolak: menambah `build_*` untuk rebalance (butuh banyak transaksi dan harga segar; tetap lewat Manage/keeper), menerima daftar tambah/hapus manager (daftar penuh lebih sederhana dan eksplisit).

## D049 — 2026-09-25 — Landing page di `/`, aplikasi pindah ke `/home`
- Masalah: `/` langsung membuka Markets; desain landing (refs/Stockbreak.html "Landing 1 of 2" + "2 of 2") belum ada.
- Keputusan:
  - Route group: `app/(landing)/page.tsx` = `/` (tanpa shell app), `app/(app)/layout.tsx` = shell (top bar, footer, tab bar mobile) untuk semua route app. Markets lama pindah ke `app/(app)/home/page.tsx` (`/home`). Root layout hanya html/body/Providers. URL route lain tidak berubah.
  - Brand di top bar app dan tab "Home" di mobile menuju `/home`; semua "Launch app" di landing menuju `/home`. Brand di header landing tetap ke `/`.
  - Landing memakai token warna yang sama (dark default, `.light` ikut), APP_NAME dari env, program ID dibaca dari `deployments/devnet.json` (fallback ID yang dipublikasikan), perintah MCP memakai `NEXT_PUBLIC_MCP_URL` atau `${origin}/api/mcp`.
  - Link yang di desain belum punya tujuan (video demo/teknis, "MCP docs") diganti: "See how it works" → `#how`, "MCP docs"/"Connect an agent" → `/agents`, video teknis dihapus.
  - Animasi tanpa library baru: CSS (`components/landing/landing.css`) + IntersectionObserver/rAF (`motion.tsx`). Hanya transform/opacity/stroke; loop berjalan hanya saat terlihat; `prefers-reduced-motion` mematikan semuanya.
- Alternatif ditolak: menyembunyikan shell di `/` lewat cek pathname di root layout (client-only, flicker); library animasi (framer-motion/motion) — bobot tambahan untuk efek yang cukup dengan CSS.

## D050 — 2026-09-25 — Deploy Cloudflare: web + cron dari GitHub (Workers Builds)
- Masalah: Vercel Hobby tidak bisa menghubungkan repo milik GitHub organization (`SkyRizzAI/stockbreak`), dan worker hanya jalan selama laptop menyala. Akun Cloudflare user punya Workers Paid (aktif sampai 16 Okt 2026) dan domain `stockbreak.fun`.
- Keputusan:
  - Dua Worker dari satu repo, di-build Workers Builds setiap push ke `main`: `stockbreak` (root `apps/web`, Next.js via `@opennextjs/cloudflare`, termasuk `/api/mcp`, Blink, OG; domain `stockbreak.fun`) dan `stockbreak-worker` (root `apps/worker`, entry `src/cf.ts`, Cron Trigger `* * * * *`).
  - Cron menjalankan loop yang sama dengan `main.ts` selama 40 detik per menit, sehingga loop < 60 s (price 15 s, indexer 2 s) tetap berdetak. Loop ≥ 60 s jalan sekali per menit atau per N menit. Oracle devnet `oracleMaxAgeSecs` = 600.
  - DB tetap Postgres (Neon) lewat Hyperdrive (binding `HYPERDRIVE`). Workers melarang socket dipakai lintas request, jadi di Workers `getDb` mengembalikan proxy yang membuka klien per request (scope OpenNext `Symbol.for("__cloudflare-context__")`, ditiru `cf.ts`). Jalur non-Workers tidak berubah.
  - `readDeployment` di Workers memakai `deployments/devnet.json` yang di-bundle (tanpa filesystem). Keypair admin/keeper dari secret `ADMIN_KEYPAIR_JSON`/`KEEPER_KEYPAIR_JSON` (baru). MCP remote memanggil API web lewat service binding `WORKER_SELF_REFERENCE` (Worker tidak bisa fetch URL publiknya sendiri, error 1042).
  - `wrangler.jsonc` di-commit tanpa rahasia (account ID, Hyperdrive ID, variabel publik). Rahasia hanya di dashboard: secret Worker dan build secret `DEVNET_DATABASE_URL` untuk migrasi di langkah build (`bun run db:remote -- migrate`, idempoten; gagal = deploy batal, versi lama tetap live).
- Risiko: `ADMIN_KEYPAIR_JSON` (upgrade authority devnet) tersimpan di akun Cloudflare. Setelah Workers Paid berakhir, batas CPU 10 ms membuat web/cron gagal. Jangan jalankan `worker:devnet` lokal bersamaan dengan cron.
- Alternatif ditolak: Vercel Hobby (org repo), deploy CLI dari laptop dengan config di luar repo (tidak otomatis), Durable Object alarm untuk loop < 1 menit (lebih kompleks dari window per cron).
