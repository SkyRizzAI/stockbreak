...
export function plainSummary(md: string): string {
  return md
    ...
    .join(" ")
    .replace(/\s+/g, " ")
    .replace(/^(?:cycle )?summary(?: for (?:the )?owner)?\s*[:—-]\s*/i, "")
    .trim();
}
...
4. If nothing needs doing, do not post (at most one short status per index per day). Finish with at most 3 short plain-text sentences that go straight to the point (what you checked, what you did or why you did nothing). No heading or label before them, no markdown, no tables, no lists.
