...
import type { IndexRef, PostItem } from "@/lib/types";
...
export function Composer({
  ...
  if (!w.address)
    return (
      <div className="flex flex-col items-start gap-3 rounded-lg border px-4 py-4 text-sm">
  ...
  const submit = async () => {
    ...
    try {
      await socialWrite<PostItem>(qc, w.address as string, "/api/posts", {
        method: "POST",
        body: JSON.stringify({ body: text, index: index?.pubkey ?? null }),
      });
      setText("");
  ...
  return (
    <div className="flex flex-col gap-2 rounded-lg border p-3">
      <Textarea
        ...
        maxLength={POST_MAX}
        placeholder={index ? `Post about ${index.symbol}…` : placeholder}
        aria-label="New post"
